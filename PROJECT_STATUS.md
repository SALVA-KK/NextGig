...
- **Implemented Auth Methods**:
  ...
  - **Google OAuth**: **Not implemented** (0 backend or frontend code).
...
- **Backend vs Frontend Verification**:
  ...
  - **Admin MFA TOTP**: Backend server-side verification in [`verify_totp_code`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/backend/apps/accounts/utils.py#L432-L463) via `pyotp.TOTP(secret).verify(code)` and [`verify_and_consume_backup_code`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/backend/apps/accounts/utils.py#L476-L499).
  - **Google ID Token / Firebase Token**: **Not implemented**.
...
- **Firebase Phone Authentication Migration (Completed)**
  - *Goal*: Replaced MSG91 SMS gateway with Firebase Phone Authentication on frontend and backend while maintaining SimpleJWT access & refresh token issuance and exact API payload contract.
  - *Backend*: Created [`apps/accounts/firebase.py`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/backend/apps/accounts/firebase.py) initializing Firebase Admin SDK with `FIREBASE_CREDENTIALS_PATH` (`config/firebase-credentials.json`). Added `verify_firebase_id_token(id_token)` verifying client ID tokens. Updated [`PhoneLoginVerifyOTPView`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/backend/apps/accounts/views.py#L654-L725) to accept Firebase ID tokens and return `{ access, refresh, user }`. Preserved `PhoneOTP` and MSG91 functions dormant for fallback/reversibility.
  - *Frontend*: Installed `firebase` SDK and created [`frontend/src/firebase.js`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/frontend/src/firebase.js). Updated [`Login.jsx`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/frontend/src/pages/auth/Login.jsx) using `RecaptchaVerifier` and `signInWithPhoneNumber`, confirming OTPs and exchanging Firebase ID tokens for JWTs via `authService.verifyPhoneLoginOTP`. Removed legacy `/phone-login/request-otp/` backend API call from client OTP dispatch.
  - *Security & Console Configuration*: Confirmed `backend/config/firebase-credentials.json` is listed in `.gitignore` and untracked by git. Noted that Firebase Console SMS Region Policy (Authentication > Settings > SMS region policy) must be set to allow testing regions (e.g. India `+91`).
