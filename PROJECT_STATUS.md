...
- **Implemented Auth Methods**:
  ...
  - **reCAPTCHA v3**: **Implemented & Active**. Invisible protection across **Registration**, **Email Login**, and **Forgot Password** forms. Uses `react-google-recaptcha-v3` on the client and server-side verification (`verify_recaptcha_token`) via `https://www.google.com/recaptcha/api/siteverify` in [`apps/accounts/utils.py`](file:///c:/Users/ACM/Desktop/myprojects/NextGig/backend/apps/accounts/utils.py#L534-L600) with a minimum score threshold of `0.5`.
- **Backend vs Frontend Verification**:
  ...
  - **reCAPTCHA v3**: Client generates token via `executeRecaptcha()`; backend validates token and score (>= 0.5) against Google siteverify API before processing form requests.
